...
function parseFetch(text, literals, seqno) {
  ...
  // Early exit if list is invalid
  if (!list || !Array.isArray(list)) {
    console.warn('Invalid fetch response structure:', list);
    return attrs;
  }

  // Helper function to determine if something is a MIME part array
  const isMimePart = (arr) => {
    return Array.isArray(arr) &&
      arr.length >= 2 &&
      typeof arr[0] === 'string' &&
      typeof arr[1] === 'string' &&
      ['TEXT', 'IMAGE', 'APPLICATION', 'MESSAGE', 'MULTIPART'].includes(arr[0].toUpperCase());
  };

  // Helper function to process MIME part
  const processMimePart = (part) => {
    try {
      return {
        type: part[0].toLowerCase(),
        subtype: part[1].toLowerCase(),
        parameters: part[2],
        id: part[3],
        filename: part[4],
        encoding: part[5],
      ...
      console.warn('Error processing MIME part:', e);
      return null;
    }
  };

  for (let i = 0, len = list.length || 0; i < len; i += 2) {
    ...
    if (!key) {
      console.warn(`Skipping null key at index ${i}`);
      continue;
    }

    if (Array.isArray(key)) {
      if (isMimePart(key)) {
        // Process MIME part instead of skipping
        const processedPart = processMimePart(key);
        if (processedPart) {
          const partKey = `attachment_${i}`;
          attrs[partKey] = processedPart;
        }
        continue;
      } else if (isParameterArray(key)) {
        // Convert parameter array to object
        const params = {};
        for (let j = 0; j < key.length; j += 2) {
          if (typeof key[j] === 'string') {
            params[key[j].toLowerCase()] = key[j + 1];
          }
        }
        attrs[`params_${i}`] = params;
        continue;
      }
      console.warn(`Skipping invalid array key at index ${i}:`, key);
      continue;
    }

    if (typeof key !== 'string') {
      console.warn(`Skipping non-string key at index ${i}:`, key);
      continue;
    }
    ...
    try {
      ...
      else if (m = RE_BODYINLINEKEY.exec(key)) {
        // Handle body inline
        try {
          val = Buffer.from('' + val);
          ...
          console.warn(`Error processing body inline for key ${key}:`, e);
          continue;
        }
      }
      attrs[key] = val;
    } catch (e) {
      console.warn(`Error processing key ${key}:`, e);
...
function parseBodyStructure(cur, literals, prefix, partID) {
  ...
  if (!cur) {
    console.warn('Received null/undefined body structure');
    return ret;
  }
  ...
  if (prefix === undefined) {
    ...
    if (Array.isArray(cur[0])) { // multipart
      ...
      try {
        ...
        console.warn('Error parsing nested body structure:', e);
      }
      ...
      if (cur[next] != null) {
        try {
          ...
          if (partLen > next) {
            if (Array.isArray(cur[next])) {
              ...
              for (i = 0, len = cur[next].length; i < len; i += 2) {
                if (cur[next][i]) {  // Add null check for parameter name
          ...
          console.warn('Error processing multipart type:', e);
          part = { type: 'unknown', params: null }
          ++next
        }
      } else {
        part = { type: 'unknown', params: null }
      ...
      try {
        if (typeof cur[1] === 'string') {
          part = {
            ...
            type: cur[0]?.toLowerCase() || 'unknown',
            subtype: cur[1]?.toLowerCase() || 'unknown',
            params: null,
            id: cur[3],
            description: cur[4],
            encoding: cur[5],
          ...
          part = {
            type: cur[0] ? cur[0].toLowerCase() : 'unknown',
            params: null
          }
          cur.splice(1, 0, null)
          ++partLen
        ...
        if (Array.isArray(cur[2])) {
          ...
          for (i = 0, len = cur[2].length; i < len; i += 2) {
            if (cur[2][i]) {  // Add null check for parameter name
        ...
        if (part.type === 'message' && part.subtype === 'rfc822') {
          ...
          if (partLen > next && Array.isArray(cur[next])) {
            try {
              ...
              console.warn('Error parsing message envelope:', e);
              part.envelope = null
            }
          } else {
            part.envelope = null
          ...
          if (partLen > next && Array.isArray(cur[next])) {
            try {
              ...
              console.warn('Error parsing nested message body:', e);
              part.body = null
            }
          } else {
            part.body = null
        ...
        if ((part.type === 'text' || (part.type === 'message' && part.subtype === 'rfc822'))
          && partLen > next) {
          part.lines = cur[next++]
        }
        ...
        console.warn('Error processing body part:', e);
        part = { type: 'unknown', params: null }
      }
    }
    ...
    try {
      ...
      console.warn('Error parsing extra structure:', e);
    }
...
function parseStructExtra(part, partLen, cur, next) {
  ...
  try {
    if (partLen > next) {
      ...
      part.disposition = disposition.type === null ? null : disposition
      ++next
    }
    ...
    console.warn('Error in parseStructExtra:', e);
  }
}
...
function parseExpr(o, literals, result = [], start = 0, useBrackets = true) {
  let inQuote = false,
    lastPos = start - 1,
    isTop = false,
    isBody = false,
...
function decodeBytes(buf, encoding, offset, mlen, pendoffset, state, nextBuf) {
  ...
  if (jsencoding.encodingExists(encoding)) {
    ...
    if (!isPartial && nextBuf) {
      ...
      let lookahead, lookaheadBuf = Buffer.alloc(buf.length + nextBuf.length)
