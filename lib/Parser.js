...
function parseExpr(o, literals, result, start, useBrackets) {
  ...
  const len = o.str.length // Defined here to ensure it's accessible everywhere within the function
  for (let i = start; i < len; ++i) {
    if (!inQuote) {
      if (isBody) {
        if (o.str[i] === ']') {
          val = o.str.substring(lastPos + 1, i)
          result.push(val)
          lastPos = i
          isBody = false
        }
        ...
        if (o.str[i] === '['
          ...
          isBody = true
          lastPos = i // Set lastPos to current to include '[' in body parsing
        } else {
          const innerResult = []
