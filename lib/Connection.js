...
class Connection extends EventEmitter {
  ...
  connect() {
    ...
    // https://gist.github.com/anonymous/ddccc12e94a1183703f97343ea240272
    ...
    parser.on('body', function (stream, info) {
      ...
      if (msg === undefined) {
        // If self._curReq is not defined, just continue
        if (!self._curReq) {
          return;
        }

        // Ensure self._curReq.fetchCache is initialized as an object
        ...
        if (self._curReq.bodyEmitter)
          self._curReq.bodyEmitter.emit('message', msg.msgEmitter, info.seqno);
        else
          console.warn('Current request bodyEmitter is undefined.');
      }
      toget = msg.toget
    ...
    parser.on('continue', function (info) {
      if (self._curReq) {
        ...
        console.error('Current request data is undefined.');
      }
    })
